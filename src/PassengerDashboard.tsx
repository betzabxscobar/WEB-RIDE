...
function HomePage({ user, activeTrip, trips, onRequest, onTrips, onCancel }: { user: User; activeTrip: Trip | null; trips: Trip[]; onRequest: () => void; onTrips: () => void; onCancel: (trip: Trip) => void }) {
  ...
  return <div className="passenger-page home-page">
    ...
    {activeTrip ? <ActiveTrip trip={activeTrip} onCancel={onCancel}/> : <section className="start-ride-card"><div className="route-mark"><i/><span/><b/></div><div><small>NUEVA SOLICITUD</small><h3>Tu viaje empieza con dos puntos</h3><p>Usa la ubicación del navegador o elige una dirección verificada en Ecuador.</p></div><button onClick={onRequest}>Definir ruta</button></section>}
...
function RequestPage({ places, origin, originPlaceId, destinationId, quote, quoting, locating, busy, activeTrip, onUseLocation, onOrigin, onDestination, onConfirm, onActive }: { places: Place[]; origin: Coordinates | null; originPlaceId: string; destinationId: string; quote: Quote | null; quoting: boolean; locating: boolean; busy: boolean; activeTrip: Trip | null; onUseLocation: () => void; onOrigin: (id: string) => void; onDestination: (id: string) => void; onConfirm: () => void; onActive: () => void }) {
  ...
  return <div className="passenger-page request-page"><div className="request-layout"><section className="request-form"><span className="passenger-kicker">DEFINE TU RECORRIDO</span><h2>Origen y destino</h2><p>El precio lo calcula Ride en el servidor según la distancia y la tarifa vigente.</p><div className="route-form"><div className="route-field origin"><i/><label>Punto de partida<select value={originPlaceId} onChange={(event) => onOrigin(event.target.value)}><option value="">Elige un lugar</option>{originPlaceId === 'gps' && <option value="gps">Mi ubicación actual</option>}{places.map((place) => <option value={place.id} key={place.id}>{place.nombre}</option>)}</select><small>{origin?.label ?? 'Puedes usar la ubicación del navegador'}</small></label></div><div className="route-line"/><div className="route-field destination"><i/><label>Destino<select value={destinationId} onChange={(event) => onDestination(event.target.value)}><option value="">¿A dónde quieres ir?</option>{places.map((place) => <option value={place.id} key={place.id}>{place.nombre}</option>)}</select><small>{places.find((place) => place.id === destinationId)?.direccion ?? 'Selecciona un destino disponible'}</small></label></div></div><button className="location-button" disabled={locating} onClick={onUseLocation}>{locating ? 'Obteniendo ubicación…' : '◎ Usar mi ubicación actual'}</button>{places.length === 0 && <p className="request-warning">No hay destinos disponibles. La administración debe cargar el catálogo de lugares.</p>}</section><aside className="quote-card"><span className="passenger-kicker">RESUMEN</span><h3>Tu cotización</h3>{quoting ? <div className="quote-loading"><i/>Calculando la mejor tarifa…</div> : quote ? <><div className="quote-price"><span>Precio estimado</span><strong>{money(quote.total)}</strong></div><dl><div><dt>Distancia estimada</dt><dd>{quote.km.toFixed(2)} km</dd></div><div><dt>Tiempo estimado</dt><dd>{quote.minutos} min</dd></div><div><dt>Tarifa</dt><dd>{quote.tarifaNombre}</dd></div></dl><button disabled={busy} onClick={onConfirm}>{busy ? 'Solicitando…' : `Confirmar por ${money(quote.total)}`}<b>→</b></button><small>El cobro definitivo se registra al finalizar el viaje.</small></> : <div className="quote-empty"><span>↗</span><p>Completa el origen y el destino para conocer el precio antes de confirmar.</p></div>}</aside></div></div>
}
